...
// Connection URL
var url = 'mongodb+srv://zliu39:QWErtyu123@clustermgdb-opdll.mongodb.net/admin?retryWrites=true&authSource=admin';
...
function insert (dbName,target,callback){
connectDb(dbName,(err,db,client)=>{
  ...
  collection.insertMany(target, function(err, result) {
    if(err)
    {
     callback("Can not insert to " + dbName, null);
     return;
    } 
    callback("",result);
    client.close();
